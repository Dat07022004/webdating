...
export const getDiscoverUsersByClerkId = async ({ clerkId }) => {
    if (!clerkId) {
...
export const handleUserActionByClerkId = async ({ clerkId, targetUserId, action }) => {
    if (!clerkId) {
        throw createServiceError(401, 'Unauthorized');
    }

    if (!targetUserId || !['like', 'pass'].includes(action)) {
        throw createServiceError(400, 'Invalid payload');
    }
...
export const getConnectionsByClerkId = async ({ clerkId }) => {
    if (!clerkId) {
        throw createServiceError(401, 'Unauthorized');
    }
