...
export function BlockListDialog({ isOpen, onClose }: BlockListDialogProps) {
  ...
  const fetchBlockedUsers = async () => {
    try {
      ...
      const res = await fetch(`${baseUrl}/api/safety/blocked`, {
        headers: {
  ...
  const handleUnblock = async (userId: string) => {
    try {
      ...
      const res = await fetch(`${baseUrl}/api/safety/unblock/${userId}`, {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${token}`,
        },
      ...
      if (data.success) {
        toast({
          title: "Đã gỡ chặn",
          description: "Người dùng đã được gỡ khỏi danh sách chặn.",
        });
      ...
      toast({
        variant: "destructive",
        title: "Lỗi",
        description: "Không thể gỡ chặn người dùng. Vui lòng thử lại.",
      });
    } finally {
        ...
        <div className="px-6 pb-6 max-h-[60vh] overflow-y-auto">
          {isLoading ? (
              ...
              {blockedUsers.map((user) => (
                <div key={user.id} className="flex items-center justify-between p-3 rounded-2xl bg-muted/30 border border-border/50">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h4 className="font-bold text-sm">{user.name}, {user.age}</h4>
                      <p className="text-[10px] text-muted-foreground">
                        Đã chặn vào {new Date(user.blockedAt).toLocaleDateString('vi-VN')}
                      </p>
                    </div>
                  </div>
                  <Button
            ...
            <div className="flex flex-col items-center justify-center py-16 text-center">
              ...
              <h3 className="font-bold text-foreground">Trống</h3>
