...
export const useChat = (activeConversationId?: string | null) => {
  ...
  const fetchConversations = useCallback(async () => {
    try {
      ...
      if (!token) return;
      const res = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:3000'}/api/chat/conversations`, {
  ...
  const fetchMessages = useCallback(async (conversationId: string) => {
    ...
    try {
      const token = await getToken();
      const res = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:3000'}/api/chat/conversations/${conversationId}/messages`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      const data = await res.json();
      if (data.success) {
  ...
  useEffect(() => {
    ...
    const handleMessagesSeen = ({ messageIds }: { messageIds: string[], conversationId: string }) => {
  ...
  return { conversations, messages, isLoading, sendMessage, fetchConversations, currentClerkId };
};
